...
%%
%% @doc Reload modified modules automatically.
...
init([]) ->
    Interval = emqx:get_env(reload_interval, 0),
    {ok, ensure_timer(Interval, #state{last = stamp()})}.

ensure_timer(Ms, State) when Ms =< 0 ->
    State;
ensure_timer(Ms, State) ->
